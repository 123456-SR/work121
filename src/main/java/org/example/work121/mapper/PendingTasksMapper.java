...
public interface PendingTasksMapper {
    ...
    @Select({"<script>",
            ...
            "  <!-- 待批准任务 (status=5) -->",
            "  <if test=\"status == '5'\">",
            "    SELECT '贝克曼梁' AS table_type, " +
                    "    t.ID AS data_id, " +
                    "    t.entrustment_id AS unified_number, " +
                    "    t.STATUS AS status, " +
                    "    t.APPROVER AS reviewer " +
                    "    FROM T_BECKMAN_BEAM t " +
                    "    LEFT JOIN T_ENTRUSTMENT e ON t.ENTRUSTMENT_ID = e.ID " +
                    "    WHERE t.STATUS = #{status} " +
                    "    AND '贝克曼梁' LIKE CONCAT('%', #{taskType}, '%') " +
                    "    UNION ALL " +
                    "    SELECT '轻型动力触探' AS table_type, " +
                    "    t.ID AS data_id, " +
                    "    NVL(e.WT_NUM, t.ID) AS unified_number, " +
                    "    t.STATUS AS status, " +
                    "    t.APPROVER AS reviewer " +
                    ...
                    "  </if>",
            "  <!-- 待审核和待提交任务 (status=1 或 status=0) -->",
            "  <if test=\"status != '5'\">",
            "    SELECT '委托单' AS table_type, " +
                    "    ID AS data_id, " +
                    "    WT_NUM AS unified_number, " +
                    "    STATUS AS status, " +
                    "    CASE WHEN #{status} = '0' THEN TESTER ELSE REVIEWER END AS reviewer " +
                    "    FROM T_ENTRUSTMENT " +
                    "    WHERE STATUS = #{status} " +
                    "    AND WT_NUM IS NOT NULL " +
                    ...
                    "    t.entrustment_id AS unified_number, " +
                    "    t.STATUS AS status, " +
                    "    CASE WHEN #{status} = '0' THEN t.FILLER ELSE t.REVIEWER END AS reviewer " +
                    "    FROM T_BECKMAN_BEAM t " +
                    "    LEFT JOIN T_ENTRUSTMENT e ON t.ENTRUSTMENT_ID = e.ID " +
                    "    WHERE t.STATUS = #{status} " +
                    "    AND '贝克曼梁' LIKE CONCAT('%', #{taskType}, '%') " +
                    ...
                    "    AND '密度试验' LIKE CONCAT('%', #{taskType}, '%') " +
                    "  </if>",
            ") t ORDER BY t.unified_number ASC",
            "</script>"})
    ...
    @Select({
            "<script>",
            "SELECT * FROM (",
            "  <!-- 状态为5的分支：查询各类试验表，按审批人筛选 -->",
            "  <if test='status != null and status.trim() == \"5\"'>",
            "    SELECT '贝克曼梁' AS table_type, ",
            "           t.ID AS data_id, ",
            "           t.entrustment_id AS unified_number, ",
            "           t.STATUS AS status, ",
            ...
            "       AND t.APPROVER = #{userAccount} ",
            "  </if>",
            "  <!-- 状态非5的分支：查询委托单+各类试验表，按填报人/审核人筛选 -->",
            "  <if test='status == null or status.trim() != \"5\"'>",
            "    SELECT '委托单' AS table_type, ",
            "           ID AS data_id, ",
            "           WT_NUM AS unified_number, ",
            "           STATUS AS status, ",
            ...
            "     WHERE STATUS = #{status} ",
            "       AND WT_NUM IS NOT NULL ",
            "       AND CASE WHEN #{status} = '0' THEN TESTER ELSE REVIEWER END = #{userAccount} ",
            "    UNION ALL ",
            "    SELECT '贝克曼梁' AS table_type, ",
            "           t.ID AS data_id, ",
            "           t.entrustment_id AS unified_number, ",
            ...
            " LEFT JOIN T_ENTRUSTMENT e ON t.ENTRUSTMENT_ID = e.ID ",
            "     WHERE t.STATUS = #{status} ",
            "       AND CASE WHEN #{status} = '0' THEN t.FILLER ELSE t.REVIEWER END = #{userAccount} ",
            "    UNION ALL ",
            "    SELECT '轻型动力触探' AS table_type, ",
            "           t.ID AS data_id, ",
            "           NVL(e.WT_NUM, t.ID) AS unified_number, ",
