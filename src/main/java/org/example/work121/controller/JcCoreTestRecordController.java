...
@RestController
@RequestMapping("/api/jc-core-test-record")
@CrossOrigin(origins = "*")
public class JcCoreTestRecordController {

    private static final Logger logger = LoggerFactory.getLogger(JcCoreTestRecordController.class);
    ...
    public Map<String, Object> getByCategory(@RequestParam String category,
        ...
        logger.info("接收到检测记录列表查询请求，CATEGORY: {}, REG_NAME: {}, WT_NUM: {}, pageNum: {}, pageSize: {}", category, regName, wtNum, pageNum, pageSize);
        ...
        try {
            ...
            // 记录表列表：仅根据记录表三种角色（填写人、记录检验人、记录审核人）进行匹配
            com.github.pagehelper.PageInfo<JcCoreWtInfo> pageInfo = jcCoreWtInfoService.getRecordsByCategory(categories, names, wtNum, pageNum, pageSize);
