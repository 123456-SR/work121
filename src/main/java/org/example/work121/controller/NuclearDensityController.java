...
@RestController
@RequestMapping("/api/nuclear-density")
@CrossOrigin(origins = "*")
public class NuclearDensityController {

    private static final Logger logger = LoggerFactory.getLogger(NuclearDensityController.class);

    @Autowired
    private NuclearDensityService service;
    ...
    public Map<String, Object> exportFormats(@RequestParam(value = "baseName", required = false) String baseName) {
        ...
        String resolvedBaseName = (baseName == null || baseName.trim().isEmpty()) ? "核子法记录表" : baseName.trim();
        try {
    ...
    public Map<String, Object> getByEntrustmentId(@RequestParam String entrustmentId) {
        Map<String, Object> result = new HashMap<>();
        try {
            java.util.List<NuclearDensity> list = service.getByEntrustmentId(entrustmentId);
            result.put("success", true);
    ...
    @PostMapping("/save")
    public Map<String, Object> save(@RequestBody NuclearDensity entity) {
        Map<String, Object> result = new HashMap<>();
        try {
            service.save(entity);
            result.put("success", true);
    ...
    public Map<String, Object> getReportByEntrustmentId(@RequestParam String entrustmentId) {
        Map<String, Object> result = new HashMap<>();
        try {
            org.example.work121.entity.NuclearDensityReport entity = service.getReportByEntrustmentId(entrustmentId);
            if (entity != null) {
                result.put("success", true);
                result.put("data", entity);
            } else {
    ...
    public Map<String, Object> saveReport(@RequestBody org.example.work121.entity.NuclearDensityReport entity) {
        Map<String, Object> result = new HashMap<>();
        try {
