...
const openExportModal = async () => {
  try {
    const res = await axios.get('/api/water-replacement/export-formats', {
...
const exportByFormat = async (format) => {
  try {
    ...
    const pageFieldRegex = /^(samplingLocation|ringVolume|initialWaterLevel|finalWaterLevel|tankArea|pitVolume|sampleMass|wetDensity|moistureNo|containerMass|wetSampleMass|drySampleMass|moistureContent|avgMoisture|measuredDryDensity|avgMeasuredDryDensity|relativeDensity)_page(\d+)_(\d+)$/
    const staticKeys = [
      'projectName', 'constructionPart', 'testDate', 'standard', 'maxDryDensity', 'minDryDensity',
      'relativeDensity', 'waterDensity', 'equipment', 'remarks', 'unifiedNumber'
    ]
    ...
    const buildPageData = (pageIndex) => {
      ...
      d.constructionPart = d.constructionPart || formData.constructionPart || currentRecord.value?.constructionPart || ''
      ...
      d.minDryDensity = d.minDryDensity || formData.minDryDensity || ''
      d.relativeDensity = d.relativeDensity || formData.relativeDensity || ''
      d.waterDensity = d.waterDensity || formData.waterDensity || ''
      d.equipment = d.equipment || formData.equipment || currentRecord.value?.equipment || ''
      d.remarks = d.remarks || formData.remarks || ''

      d.entrustingUnit = currentRecord.value?.clientUnit || formData.entrustingUnit || dynamicData.entrustingUnit || ''
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        const response = await axios.post('/api/water-replacement/export-excel', {
          id,
          entrustmentId,
          pageNo,
          totalPages: total,
