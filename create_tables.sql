...
-- 1. 删除 POSITION 字段
ALTER TABLE JZS_USER_EXT
DROP COLUMN POSITION;

-- 2. 新增四个权限字段（默认值为 'N'，表示无权限；'Y' 表示有权限）
ALTER TABLE JZS_USER_EXT
    ADD (
    APPROVAL_PERMISSION  VARCHAR2(1 BYTE) DEFAULT 'N' NOT NULL,  -- 审批权限
    AUDIT_PERMISSION     VARCHAR2(1 BYTE) DEFAULT 'N' NOT NULL,  -- 审核权限
    SUBMIT_PERMISSION    VARCHAR2(1 BYTE) DEFAULT 'N' NOT NULL,  -- 提交权限
    ASSIGN_PERMISSION    VARCHAR2(1 BYTE) DEFAULT 'N' NOT NULL   -- 分配权限
);

-- 3. 给字段加注释（方便后续维护）
COMMENT ON COLUMN JZS_USER_EXT.APPROVAL_PERMISSION IS '审批权限(Y:有权限,N:无权限)';
COMMENT ON COLUMN JZS_USER_EXT.AUDIT_PERMISSION IS '审核权限(Y:有权限,N:无权限)';
COMMENT ON COLUMN JZS_USER_EXT.SUBMIT_PERMISSION IS '提交权限(Y:有权限,N:无权限)';
COMMENT ON COLUMN JZS_USER_EXT.ASSIGN_PERMISSION IS '分配权限(Y:有权限,N:无权限)';
